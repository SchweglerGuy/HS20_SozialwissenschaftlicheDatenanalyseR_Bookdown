...
Folgende Befehlszeilen wurden in einem Skript namens "ess_import.R" in einem Unterordner des Arbeitsverzeichnisses abgespeichert (vgl. für die Angaben zu den fehlenden Werten Aufgabe 4 aus dem WP07):
```{r eval=FALSE}
#Laden der Daten
...
Das R-Skript kann nun laufend ergänzt werden, etwa der Definition von weiteren fehlenden Werte bei anderen Variablen, mit der Definition von Faktoren (siehe nächster WP), usw. Dies hat den Vorteil, dass zu Beginn einer neuen Einheit (d.h. wenn Sie ein neues Markdown erstellen) jeweils nicht der ganze Code nochmals aufgeführt werden muss, sondern dass direkt dieses Skript ausgeführt werden kann. Ein Ausführen eines Skriptes (oder auch einer sonstigen Textdatei) erlaubt uns die Funktion 'source()'.
```{r eval=FALSE}
...
Wenn wir Tabellen (bzw. Kontingenztabellen) darstellen, so ist die Konvention dass "bei gerichteten Beziehungen die Ausprä-gungen der unabhängigen Variablen (X) den Spalten zugeordnet werden und die Ausprägungen der abhängigen Variablen (Y) den Reihen zugeordnet werden" [@DiazBone2019, 70]. Die Aufgabenstellung spricht zwar nicht von einer gerichteten Beziehung, aber gleichzeitig macht es Sinn, Geschlecht als unabhängige Variable anzusehen (und nicht dass das Interesse an Politik das Geschlecht einer Person bestimmt). Gemäss dieser Überlegung sollen dann auch die Randsummen ausgerichtet werden, da uns die Verteilungen des Politikinteresses je nach Geschlecht interessiert. Das heisst das pro Geschlecht jeweils auf 100% summiert werden soll.
...
table(gndr, polintr)
prop.table(table(gndr, polintr))
prop.table(table(gndr, polintr),1)
addmargins(prop.table(table(gndr, polintr),1))
addmargins(prop.table(table(gndr, polintr),1),2)
round(addmargins(prop.table(table(gndr, polintr),1),2),2)
```
...
```{r eval=FALSE}
CT_Ergebnis <- CrossTable(gndr, polintr, chisq = T)
```
