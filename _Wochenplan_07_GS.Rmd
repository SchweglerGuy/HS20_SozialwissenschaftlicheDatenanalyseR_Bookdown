...
Über die 'summary()' wir dann nochmals deutlich, dass hier tatsächlich Rohdaten eingelesen wurden, da beispielsweise fehlende Werte noch als Zahlen gelten (und so etwa jemand im Jahr 7777 geboren wäre).
```{r}
...
Der oben beschriebene Fall ist nun nicht ein Fehler im Datensatz, sondern vielmehr ein fehlender Wert, was das Codebuch ausweist. Fehlende Werte können wie folgt definiert werden:
```{r}
...
#Dieses Definieren von fehlende WErten kann auch für die drei Variablen zu Bildungsabschlüssen auf ähnliche Weise erfolgen:
...
Auf der *zweiten Stufe* überprüfen wir nun, ob die Daten auch wirklich Sinn machen, das heisst im Rahmen der erhobenen Daten. Ein Vergleich mit dem Codebuch zeigt dabei, dass auf der einen Seite erst Personen ab dem 15.Lebensjahr befragt wurden. Folgender Fall macht also keinen Sinn:
```{r}
which(daten_ess$agea<15)
```
Die bei der Variable eisced auftauchende Ausprägung '0' ist durchaus zulässig und entspricht "Not possible to harmonise into ES-ISCED".

Auf der *dritten Stufe* können wir dann mittels Kreuztabellierungen und logischen Überlegungen jeweils (mindestens) zwei Variablen miteinander vergleichen, um so nochmals Fehler zu entdecken. Zuerst können wir ein zusätzliche Variable als Fehlerindikator der Angaben zum Alter berechnen und dann als logische Bedingungen nutzen, um drei fehlerhafte Fälle zu entdecken:
```{r}
...
Beim Umgang mit diesen fehlenden Werten haben wir drei Möglichkeiten. Wir können die benötigen Angaben von anderen Werten ableiten, zum Beispiel das Alter vom Geburtsjahr. 
```{r}
daten_ess$agea[132] <- 2016 - daten_ess$yrbrn[132]

```

Weiter können wir uns einen Werte berechnen bzw. diesen schätzen:
```{r}
daten_ess$eduyrs[1482] <- round(mean(daten_ess$eduyrs[daten_ess$edulvla==3],
                                     na.rm = T))
```

Die letzte Option ist dann noch die falschen Angaben als fehlende Werte zu definieren:
```{r}
...
Um den zufälligen Datensatz mit 200 Fällen zu erstellen bieten sich mindestens zwei Möglichkeiten an. Die erste Möglichkeit nutzt die 'sample()' Funktion und wählt zufällig 200 Zahlen einer Variable aus, die von 1 bis 1525 durchläuft. Eine solche Variable ist 'x' im Datensatz -- oder auch einfach die Reihenzahl. Die ausgewählten Zahlen können als Angabe für die Zeilen in die eckige Klammerfunktion eingesetzt werden.
```{r}
...
Die zweite Möglichkeit berechnet eine Zufallsvariable und sortiert den Datensatz dann nach der Grösse dieser Zufallsvariable. Anschliessend können dann die die -- jetzt völlig zufällig -- ersten 200 Fälle ausgewählt werden.
```{r}
