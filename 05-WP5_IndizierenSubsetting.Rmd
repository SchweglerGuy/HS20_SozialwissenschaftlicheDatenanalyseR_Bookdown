...
Für den folgenden Datensatz wurden Beispiel aus der letzten Falllösung genommen (WP04, Aufgabe 1).
```{r}
Geschlecht <- c(rep(c(1,0),50))
Einkommen_CHF <- c(seq(4100,14000,100))
AnzahlZimmer_H <- c(rep(2, 5), rep(1, 7), rep(4, 18), rep (5, 35), rep(3, 25), rep(6, 10))
Schulzeit_J <- c(1,1,1,1, rep(seq(from=2, to=9), each=11),10,10,10,10,10,10,10,10)
...
dim(datensatz)
```
...die Dimensionen Ihres Datensatzes aus, das heisst die Zeilen- und Spaltenzahl, was der Anzahl Fällen und Variablen entspricht;

```{r}
names(datensatz)
```
...die Namen Ihrer Vektoren aus, also die Variablennamen;

```{r}
str(datensatz)
```
...die Struktur Ihres Datensatzes aus, das heisst die Unterobjekte, auf die Sie mittels dem Dollarzeichen zugreifen können.

```{r}
class(datensatz)
```
...die Klasse Ihres Objektes Datensatzes aus, das heisst die von Ihnen zugewiesene Eigenschaft des Objektes;
...
Der Zusatz '(dim(datensatz)[2]-1))' generiert lediglich die Zahl 4. Die for-Schleife soll ja den Mittelwerte für die vier numerischen Variablen des Datensatzes berechnen. Dies könnte man auch einfach hinschreiben ('i in 1:4') oder mit verschiedenen weiteren Möglichkeiten erreichen (etwa über '(ncol(datensatz)-1)').
...
```
Die erste Variante generiert zwei Teildatensätze, wobei in ersterem alle mit durchschnittlichem Einkommen enthalten sind und im zweiten diejenigen, die entweder über ein unter- oder überdurchschnittliches Einkommen verfügen.
